...
def fill_whole_limits(side: Side, order: Order, result: ResultBuilder) -> bool:
    ...
    while order.quantity() > 0 and not side.empty():
        ...
        if order.quantity() < lim.volume():
            return False  # if can not match whole limits anymore, break

        # update result object
        result.inc_orders_matched(lim.valid_orders())
        result.inc_execprices(lim.price(), lim.volume())

        order.fill(lim.volume(), lim.price())  # partially fill order with limit volume
        side.update_volume(
            -lim.volume()
        )  # substract limit volume from side volume before filling all orders in limit
...
def fill_whole_orders(side: Side, order: Order, result: ResultBuilder) -> bool:
    ...
    while order.quantity() > 0:
        ...
        order.fill(next_order.quantity(), lim.price())
        lim.fill_next(next_order.quantity())
...
def fill_last_order(side: Side, order: Order, result: ResultBuilder):
    ...
    if order.valid():
        ...
        order.fill(order.quantity(), lim_order.price())
...
def oop(order: Order, lim_price: Decimal) -> bool:
    ...
    match order.side():
        case OrderSide.BID:
            return order.price() < lim_price
        case OrderSide.ASK:
            return order.price() > lim_price


def mk_oop_msg(p, q):
    return f"<matching engine>: order out-of-price at ({p}), quantity left: ({q})"
