...
class Side(abc.ABC):
    ...
    def cancel_order(self, order: Order) -> None:
        ...
        if lim.empty():
            del self._price2limits[lim.price()]
    ...
    def delete_fakeorder(self, price: Decimal):
        ...
        if not limit.fakeorder_exists():
            return

        limit.delete_fakeorder()
        if limit.volume() == 0.0:
            self.pop_limit(price)
...
class BidSide(Side):
    ...
    def is_market(self, order: AskOrder) -> bool:
        if self.empty():
            return False
        if self.best().price() >= order.price():
            return True
        return False
...
class AskSide(Side):
    ...
    def view(self, n: int = 10) -> str:
        ...
        buffer = io.StringIO()
        if self.size() > n:
            buffer.write(f"   ...({self.size() - n} more asks)\n")
        count = 0
        l = list()
