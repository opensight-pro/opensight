...
class OrderParams:
    ...
    def __init__(
        ...
        OrderParams.check_args(side, price, quantity, is_market, otype, expiry)
    ...
    def check_args(
        ...
        if not isinstance(is_market, bool):
            raise TypeError(f"is_market should be of type bool but is {type(is_market)}")

        if not isinstance(quantity, Number):
            raise TypeError(
                f"quantity should be of type Number but is {type(quantity)}"
            )

        if not isinstance(otype, OrderType):
            raise TypeError(
                f"ordertype should be of type OrderType but is {type(otype)}"
            )
        ...
        if otype == OrderType.GTD:
            if expiry is None:
                raise ValueError("order is GTD but expiry is None")

            expiry = int(expiry)
            now = ceil(time.time())
            if expiry <= now:
                raise ValueError(
                    f"order expiry ({expiry}) is less than current timestamp ({now}), or too close"
                )
        ...
        if quantity_decimal < TICK_SIZE_QTY:
            raise ValueError(
                f"quantity ({quantity}) must be greater than {TICK_SIZE_QTY}"
            )
    ...
    def __repr__(self) -> str:
        return (
            f"OrderParams(side={self.side.name}, price={self.price}, qty={self.quantity}, "
